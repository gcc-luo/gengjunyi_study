...
export function AppStoreProvider({ children, initialSnapshot }: { children: ReactNode; initialSnapshot?: Snapshot }) {
  ...
  const value = useMemo<AppStoreValue>(() => {
    ...
    const updateCourse = (id: string, input: Partial<CourseInput>) => { const course = snapshot.courses.find((item) => item.id === id); if (!course) return undefined; const next = { ...course, ...input, updatedAt: now() }; update((draft) => { const index = draft.courses.findIndex((item) => item.id === id); draft.courses[index] = next; }); return next; };
    const publishCourse = (id: string): PublishResult => { const course = snapshot.courses.find((item) => item.id === id); if (!course) return { ok: false, reason: '课程不存在' }; const result = canPublishCourse(course, snapshot.videos); if (!result.ok) return result; update((draft) => { const target = draft.courses.find((item) => item.id === id); if (target) { target.status = CourseStatus.PUBLISHED; target.updatedAt = now(); } }); return { ok: true }; };
    const offlineCourse = (id: string) => { const course = snapshot.courses.find((item) => item.id === id); if (!course) return undefined; const next = { ...course, status: CourseStatus.OFFLINE, updatedAt: now() }; update((draft) => { const target = draft.courses.find((item) => item.id === id); if (target) Object.assign(target, next); }); return next; };
    ...
    const updateVideo = (id: string, input: Partial<VideoInput>) => { const video = snapshot.videos.find((item) => item.id === id); if (!video) return undefined; const next = { ...video, ...input }; update((draft) => { const index = draft.videos.findIndex((item) => item.id === id); draft.videos[index] = next; }); return next; };
    ...
    const updateChild = (id: string, input: Partial<ChildInput>) => { const child = snapshot.children.find((item) => item.id === id); if (!child) return undefined; const next = { ...child, ...input }; update((draft) => { const index = draft.children.findIndex((item) => item.id === id); draft.children[index] = next; }); return next; };
    const deactivateChild = (id: string) => { const child = snapshot.children.find((item) => item.id === id); if (!child) return undefined; const next = { ...child, status: ChildStatus.INACTIVE }; update((draft) => { const target = draft.children.find((item) => item.id === id); if (target) target.status = ChildStatus.INACTIVE; }); if (currentChildId === id) setCurrentChildId(null); return next; };
    const saveWatchProgress = (input: ProgressInput) => { const previous = snapshotRef.current.watchProgress.find((item) => item.childId === input.childId && item.videoId === input.videoId); const nextProgress = createProgressUpdate(previous, input); update((draft) => { const index = draft.watchProgress.findIndex((item) => item.childId === input.childId && item.videoId === input.videoId); if (index === -1) draft.watchProgress.push(nextProgress); else draft.watchProgress[index] = nextProgress; draft.watchEvents = addWatchEvent(draft.watchEvents, { childId: input.childId, videoId: input.videoId, isPlaying: input.isPlaying, deltaWatchSeconds: input.deltaWatchSeconds, positionSeconds: input.lastPositionSeconds, occurredAt: input.updatedAt }); }); return nextProgress; };
