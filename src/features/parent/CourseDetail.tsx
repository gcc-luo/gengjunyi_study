...
export function CourseDetail() {
  ...
  const { courses, videos, subjects, updateVideo, updateCourse, publishCourse, offlineCourse, removeVideo, restoreVideo } = useAppStore();
  ...
  const recoverVideo = async (video: typeof videos[number]) => {
    setBusy(true);
    try { await restoreVideo(video.id); setError(''); }
    catch (cause) { setError(cause instanceof Error ? cause.message : '恢复视频失败，请重试'); }
    finally { setBusy(false); }
  };
  ...
  return <div className="parent-page">
    ...
    <Modal open={Boolean(deleteTarget)} title="确认归档视频" onClose={() => setDeleteTarget(undefined)}><p>归档“{deleteTarget?.title}”后儿童端将不再显示，但学习进度、收藏和观看记录会保留，之后可以恢复。</p><div className="form-actions"><button type="button" className="button" disabled={busy} onClick={() => setDeleteTarget(undefined)}>取消</button><button type="button" className="button danger" disabled={busy} onClick={() => void deleteVideo()}>{busy ? '归档中…' : '归档视频'}</button></div></Modal>
    <Modal open={Boolean(editingVideo)} title="编辑视频标题" onClose={closeVideoEditor}><form className="editor-form video-title-form" onSubmit={(event) => void saveVideoTitle(event)}><div className="video-title-field"><label htmlFor="video-title-input">视频标题</label><span className="video-title-hint">标题会显示在课程目录中，建议使用简洁清晰的名称</span><input id="video-title-input" className="video-title-input" aria-label="视频标题" aria-describedby="video-title-hint" placeholder="例如：第1课 静夜思" value={videoTitle} onChange={(event) => setVideoTitle(event.target.value)} /></div><div className="form-actions"><button type="button" className="button" disabled={busy} onClick={closeVideoEditor}>取消</button><button type="submit" className="button primary" disabled={busy}>{busy ? '保存中…' : '保存视频'}</button></div></form></Modal>
  </div>;
}
