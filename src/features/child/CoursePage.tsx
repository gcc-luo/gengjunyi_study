...
export function CoursePage() {
  ...
  const orderedVideos = course.videoIds.map((id) => videos.find((video) => video.id === id)).filter((video): video is NonNullable<typeof video> => Boolean(video)).sort((a, b) => a.orderIndex - b.orderIndex);
  ...
  return <main className="child-page course-detail-page"><Link className="child-back-link" to="/child/courses">← 返回课程</Link><section className="course-detail-hero"><div className="large-course-cover detail-course-cover" style={{ background: `linear-gradient(135deg, ${course.cover.colors.join(',')})` }}><span>{subject?.icon ?? '✦'}</span><small>{subject?.name ?? course.subjectId}</small></div><div className="course-detail-copy"><p className="child-kicker">{subject?.name ?? course.subjectId} · {course.ageRange || '适合所有小朋友'}</p><h1>{course.title}</h1><p>{course.description || '跟着课程一起发现新知识。'}</p><div className="course-meta"><span>{orderedVideos.length} 集内容</span><span>已完成 {Math.round(progress * 100)}%</span></div><ProgressBar value={progress} label={`${course.title}课程完成度`} /></div></section><div className="course-tabs" role="tablist"><button type="button" role="tab" aria-selected={tab === 'catalog'} className={tab === 'catalog' ? 'active' : ''} onClick={() => setTab('catalog')}>课程目录</button><button type="button" role="tab" aria-selected={tab === 'intro'} className={tab === 'intro' ? 'active' : ''} onClick={() => setTab('intro')}>课程介绍</button></div>{tab === 'intro' ? <section className="course-intro panel"><h2>关于这门课</h2><p>{course.description || '这是一门为小朋友准备的趣味课程，按自己的节奏学习每一集内容吧。'}</p><div className="intro-note">适龄：{course.ageRange || '所有年龄'}<br />共 {orderedVideos.length} 集，每一集都可以自由选择。</div></section> : <section className="catalog-list">{orderedVideos.length ? orderedVideos.map((video, index) => { const item = progressFor(video.id); const completed = Boolean(item?.completed); const percent = item ? Math.round(item.maxProgress * 100) : 0; return <div className={`catalog-row ${completed ? 'is-complete' : ''}`} data-testid={`video-row-${video.id}`} key={video.id}><span className="catalog-number">{completed ? '✓' : index + 1}</span><span className="catalog-main"><strong>{video.title}</strong><small>{durationText(video.durationSeconds)}{completed ? ' · 已完成' : percent ? ` · 已学习 ${percent}%` : ''}</small></span>{video.status === VideoStatus.READY ? <Link className="catalog-play" to={`/child/watch/${video.id}`} aria-label={`播放 ${video.title}`}>{completed ? '再看一次' : item ? '继续学习' : '开始学习'} <span aria-hidden="true">→</span></Link> : <span className="catalog-unavailable">暂不可播放</span>}</div>; }) : <EmptyState title="课程还没有视频" description="请让家长为课程添加视频内容。" />}</section>}</main>;
}
