...
export function ChildShell() {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener(EYE_CARE_EVENT, syncPreference); window.removeEventListener('storage', syncPreference); };
  }, []);

  return <div className="child-stage"><div className={`child-shell${eyeCare ? ' eye-care' : ''}`} data-testid="child-shell"><header className="child-topbar"><Link to="/child/select" aria-label="切换孩子">小小学习星球</Link><span aria-hidden="true">☼</span></header><div className="child-content"><Outlet /></div>{currentChildId && <nav className="child-bottom-nav"><NavLink to="/child/home"><Icon name="home" size={21} />首页</NavLink><NavLink to="/child/courses"><Icon name="book" size={21} />课程</NavLink><NavLink to="/child/me"><Icon name="child" size={21} />我的</NavLink></nav>}</div></div>;
}
