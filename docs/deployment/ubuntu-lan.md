...
脚本会检查 Docker Engine 与 Compose 插件，生成本机 `.env.lan`，构建并启动 Web、API、PostgreSQL 和 MinIO，初始化数据库/私有视频桶，并创建默认家长管理员。数据库和视频使用独立的 Docker 命名卷保存。应用默认启用全局免登录：浏览器首次打开时，API 会为现有家长管理员自动创建会话。
...
首页的两个卡片分别直达家长管理端和儿童端；同一局域网内的手机浏览器输入主站地址即可。若脚本选错网卡，可查看 `ip -4 addr`，使用服务器实际的局域网 IPv4 重跑：
...
请替换为你自己的公网入口。脚本会保存这两个地址，后续重跑时会保留；若更换公网入口，重新设置这两个环境变量再运行。公网 HTTP 会明文传输所有请求数据，而且免登录意味着任何访问者都可以管理家庭数据、上传或删除视频；不应将此地址暴露到公网或不可信网络。
...
此部署默认启用全局免登录（`AUTH_BYPASS=true`），没有访客/儿童权限隔离；每个访问者都拥有家长权限。该模式只适合可信、隔离的家庭网络。若通过公网或内网穿透暴露服务，任何知道地址的人都可查看和修改家庭数据。
