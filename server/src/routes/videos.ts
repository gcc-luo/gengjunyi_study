...
import { deleteVideo, permanentlyDeleteVideoFiles, restoreVideo, retryVideoProcessing, UploadError } from "../services/uploads.js";

const paramsSchema = z.object({ videoId: z.string().min(1).max(128) });
const deleteSchema = z.object({ confirmHistoryDeletion: z.literal(true).optional(), permanent: z.literal(true).optional() }).optional().default({});
...
export function registerVideoManagementRoutes(
  ...
  app.delete("/api/videos/:videoId", { preHandler: app.requireParentUnlocked }, async (request, reply: FastifyReply) => {
    ...
    try {
      if (body.data.permanent) await permanentlyDeleteVideoFiles(prisma, storage, { videoId: params.data.videoId });
      else await deleteVideo(prisma, storage, { videoId: params.data.videoId });
      return reply.code(204).send();
    } catch (error) {
      if (!(error instanceof UploadError)) throw error;
      const status = error.code === "video-not-found" ? 404 :
        error.code === "course-is-published" || error.code === "video-processing" ? 409 : 400;
